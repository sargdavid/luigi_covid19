...
```{r}
tmp <- data.table(sample_names = colnames(dt_rna_cod)[11:ncol(dt_rna_cod)],
                  BAMBI = dt_rna_cod[gene_name == "BAMBI", c(11:ncol(dt_rna_cod))])
tmp <- merge(rnaseq_meta,
             tmp,
             by = "sample_names")

tmp2 <- fread("Data/COVID study Master sheet w ELISA data 2_1-24.csv")

tmp2$`Real ID` <- as.character(tmp2$`Real ID`)
...
tmp$`ACE2 (ng/ml)` <- as.numeric(tmp$`ACE2 (ng/ml)`)
...
```{r}
plot(tmp$BAMBI ~ log10(tmp$`ACE2 (ng/ml)` + 1))

ggplot(tmp,
       aes(x = log10(`ACE2 (ng/ml)` + 1),
           y = BAMBI,
           fill = `Study Group`)) +
  geom_point(shape = 21,
               size = 3) +
