...
*ToDos:*  
1.	Transform the data using quantile or Fisher-Yates normalization (DNAMR::cnormalize), and perform factor analysis (psych::fa instead of factanal)  
2.	Apply tow-part model to normalized gene expressions.  
3.	Reactom Pathway Analysis (ReactomPA) repeated on these sets of genes  
4.	Factor analysis of gene expressions. Compare the factors with Gene Ontology, Reactom Pathways, KEGG pathways etc. Include Module Analysis (see WGCNA: Weighted Correlation Network Analysis by Steve Horvath. We used it in lncRNA paper. Can be more applicable for Paper 2 on lncRNAs.)  

**Feedbakc from stats AI meeting 3/8/2024:**  
1. Use MOFA (Bernard)  
2. Check Diabolo and COMBI packages  
...
5. Factor analysis  
6. Gene ontology to connect factors to GOs (Javier) 
7. Ask Lu if I can share Table 1 with Michael Katehakis  

**Meeting 2/1/2024:**   
1. RNA vs. ELISA - need to know coding genes  
2. Neanderthal genes  
3. List of genes from Lu  
4. IFI27: what cyto/ cemokine is associated with it (from ELISA panel)? Cytokines should regulate IFI27  
5. BAMBI vs ACE2 (new ELISA)  
6. Find specific proteins (ELISA) that result in the expression of another gene  
7. Pathway analysis with ReactomPA (DONE)  
8. Focus on DM  
9. Factor analysis  
10. Anti-IL6 was used in pandemic. Look at INFg, IL1, IL6 and TNFa  
...
dt_elisa_l$Log10Conc <- log10(dt_elisa_l$`Concentration[pg/mL]` + 1)
```
...
ggplot(dt_elisa_l,
  ...
  geom_point(aes(fill = grp,
             ...
             position = position_dodge(0.3)) +
  scale_x_discrete("") +
  scale_y_discrete("Log10(Concentration[pg/mL] + 1)") +
  theme_bw() +
  theme(legend.position = "none",
...
```{r}
foo <- function(dx) {
  return(tpm(formula_part1 = Log10Conc ~ `Study Group`*AnyDM,
             formula_part2 = NULL,
             link_part1 = "logit",
...
m1 <- dt_elisa_l[, .(models = list(tpm(formula_part1 = Log10Conc ~ grp,
                                       formula_part2 = NULL,
                                       link_part1 = "logit",
                                       family_part2 = gaussian(link = "identity"),
                                       data = data.table(Log10Conc = Log10Conc,
                                                         grp = grp)))),
                 by = Protein]
```
...
```{r}
m1 <- factanal(x = dt_elisa[, 4:ncol(dt_elisa)],
               factors = 1)

m1

m2 <- factanal(x = dt_elisa[, 4:ncol(dt_elisa)],
               factors = 2)

m2

m3 <- factanal(x = dt_elisa[, 4:ncol(dt_elisa)],
               factors = 3)
