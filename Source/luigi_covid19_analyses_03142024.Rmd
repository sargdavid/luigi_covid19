...
Analysis data sets and copy of the Rmd are in Rutgers box:\
...
1.  Add p-values to tables
2.  Make WHO OSCI plots. See Lu's and Marshall's examples.
3.  Note: ACE2 inhibitor is an entry point. Check for significant differences in DPPIV and ACE2.
4.  Add average estimates forest plots for ELISA.
...
2.  What are the profiles of protein-non-producing patients?
...
5.  Neanderthals' genes (see Lu's email). <https://genome.ucsc.edu/Neandertal/>
...
```{r}
foo1 <- function(a = tmp2$`Inpatient=1, ER=2, OP=3`,
                 b = tmp2$`Study Group`,
                 c = tmp2$AnyDM,
                 c_name = "") {
  tmp1 <- data.table(a, b, c)
  
  tmp1$d <- paste(tmp1$b,
                  tmp1$c, 
                  sep = "/")
  
  tmp1$d <- factor(tmp1$d,
                   levels = c("No COVID/0",
                              "No COVID/1",
                              "COVID/0",
                              "COVID/1"))
  
  if(class(tmp1$a) == "factor") {
    t1 <- tmp1[, .(`No COVID/No DM` = paste0(sum(d == "No COVID/0"),
                                             " (",
                                             round(100*sum(d == "No COVID/0")/.N, 1),
                                             "%)"),
                   `No COVID/DM` = paste0(sum(d == "No COVID/1"),
                                          " (",
                                          round(100*sum(d == "No COVID/1")/.N, 1),
                                          "%)"),
                   `COVID/No DM` = paste0(sum(d == "COVID/0"),
                                          " (",
                                          round(100*sum(d == "COVID/0")/.N, 1),
                                          "%)"),
                   `COVID/DM` = paste0(sum(d == "COVID/1"),
                                       " (",
                                       round(100*sum(d == "COVID/1")/.N, 1),
                                       "%)"),
                   `p-Value` = round(chisq.test(x = tmp1$a,
                                                y = tmp1$d)$p.value,
                                     3)),
               by = a]
    ...
    t1 <- tmp1[, .(c_name,
                   `No COVID/No DM` = paste0(round(mean(a[d == "No COVID/0"],
                                                        na.rm = TRUE), 1),
                                             " (",
                                             round(sd(a[d == "No COVID/0"],
                                                      na.rm = TRUE), 1),
                                             ")"),
                   `No COVID/DM` = paste0(round(mean(a[d == "No COVID/1"],
                                                     na.rm = TRUE), 1),
                                          " (",
                                          round(sd(a[d == "No COVID/1"],
                                                   na.rm = TRUE), 1),
                                          ")"),
                   `COVID/No DM` = paste0(round(mean(a[d == "COVID/0"],
                                                     na.rm = TRUE), 1),
                                          " (",
                                          round(sd(a[d == "COVID/0"],
                                                   na.rm = TRUE), 1),
                                          ")"),
                   `COVID/DM` = paste0(round(mean(a[d == "COVID/1"],
                                                  na.rm = TRUE), 1),
                                       " (",
                                       round(sd(a[d == "COVID/1"],
                                                na.rm = TRUE), 1),
                                       ")"),
                   `p-Value` = round(summary(lm(a ~ d,
                                                data = tmp1))$coefficients[2, 4],
                                     3))]
    t1$`p-Value`[t1$`p-Value` < 0.001] <- "<0.001"
  }
  t1 <- data.table(Variable = c_name,
                   t1)
  return(t1)
}

foo2 <- function(a = tmp2$`Inpatient=1, ER=2, OP=3`,
                 c = tmp2$AnyDM,
                 c_name = "") {
  tmp1 <- data.table(a,  c)
  
  if(class(tmp1$a) == "factor") {
    t1 <- tmp1[, .(`COVID/No DM` = paste0(sum(c == 0),
                                          " (",
                                          round(100*sum(c == 0)/.N, 1),
                                          "%)"),
                   `COVID/DM` = paste0(sum(c == 1),
                                       " (",
                                       round(100*sum(c == 1)/.N, 1),
                                       "%)"),
                   `p-Value` = round(chisq.test(x = tmp1$a,
                                                y = tmp1$c)$p.value,
                                     3)),
    ...
    t1 <- tmp1[, .(c_name,
                   `COVID/No DM` = paste0(round(mean(a[c == 0],
                                                     na.rm = TRUE), 1),
                                          " (",
                                          round(sd(a[c == 0],
                                                   na.rm = TRUE), 1),
                                          ")"),
                   `COVID/DM` = paste0(round(mean(a[c == 1],
                                                  na.rm = TRUE), 1),
                                       " (",
                                       round(sd(a[c == 1],
                                                na.rm = TRUE), 1),
                                       ")"),
                   `p-Value` = round(summary(lm(a ~ c,
                                                data = tmp1))$coefficients[2, 4],
                                     3))]
    t1$`p-Value`[t1$`p-Value` < 0.001] <- "<0.001"
  }
...
Subject demographic and clinical characteristics stratified by the presence and absence of diabetes and COVID-19.

```{r}
tmp2 <- dt1[, c("Study Group",
                "AnyDM",
...
for(i in 3:ncol(tmp2)) {
  out[[i - 2]] <- foo1(a = tmp2[[i]],
                       c_name = colnames(tmp2)[i])
}
```
...
Available patient clinical laboratory values upon hospital presentation.
...
Comparison of medication usage between patient with and without COVID-19 and diabetes
...
Patient hospital disposition after admission
...
tmp2 <- dt1[, c("Study Group",
                ...
                "ICU LOS",
                "LOS",
                # "Still_In_Hospital_08182023",
                "Expired")]

out4 <- list()
for(i in 3:ncol(tmp2)) {
  out4[[i - 2]] <- foo1(a = tmp2[[i]],
                        c_name = colnames(tmp2)[i])
}
```
...
Comparison of disease progression with the WHO Ordinal Scale for Clinical Severity

```{r}
tmp2 <- droplevels(dt1[`Study Group` == "COVID",
                       c("AnyDM",
                         "WHO-OSCI D1",
                         "WHO-OSCI D3",
                         "WHO-OSCI D7",
...
Plasma inflammatory cytokine and chemokine comparison between subjects with and without COVID-19 and diabetes mellitus
...
out6 <- list()
for(i in 3:ncol(tmp2)) {
  out6[[i - 2]] <- foo1(a = log10(tmp2[[i]] + 1),
                        c_name = colnames(tmp2)[i])
}
```
...
World Health Organization Ordinal Scale for Clinical Severity
<5 = mild
>= 5 = severe
5 = very sick
6 = intubated
...
### WHO-OSCI over time
Source: https://stackoverflow.com/questions/10834382/keep-unused-levels-in-bar-plot  
Source: https://stackoverflow.com/questions/3472980/how-to-change-facet-labels 

```{r}
tmp <- dt1[`Study Group` == "COVID",
           c("Real ID",
             "AnyDM",
             "Expired",
             "WHO-OSCI D1",
...
tmp1 <- melt.data.table(tmp,
                        id.vars = c(1:3),
                        variable.name = "Day",
                        value.name = "WHO_OSCI")
tmp1$Day <- factor(tmp1$Day,
                   levels = unique(tmp1$Day))

tmp1 <- tmp1[!is.na(tmp1$WHO_OSCI), ]
...
tmp2 <- dcast.data.table(tmp2,
                         Day + AnyDM ~ WHO_OSCI,
                         value.var = "N")

tmp2 <- melt.data.table(tmp2,
                        id.vars = 1:2,
                        measure.vars = 3:11,
                        variable.name = "WHO_OSCI",
                        value.name = "N")
```
...
p1 <- ggplot(tmp2,
  ...
  facet_wrap(~ AnyDM,
             ncol = 1,
             scale = "free_y",
             labeller = as_labeller(c("0" = "No DM",
                                      "1" = "DM"))) +
  ...
  geom_bar(position = "dodge",
           color = "black",
           stat = "identity") +
  scale_x_discrete("WHO OSCI") +
  scale_fill_discrete(breaks = levels(tmp2$Day),
                      labels = gsub(x = levels(tmp2$Day),
                                    pattern = "WHO-OSCI ",
                                    replacement = "")) +
  theme_bw() +
  theme(legend.position = "bottom")

tiff(filename = "../tmp/day_over_who_osci.tiff",
     height = 6,
     width = 6,
     units = 'in',
     res = 600,
...
p2 <- ggplot(tmp2,
  ...
  scale_x_discrete("Day",
                   breaks = levels(tmp2$Day),
                   labels = gsub(x = levels(tmp2$Day),
                                 pattern = "WHO-OSCI ",
                                 replacement = "")) +
  scale_fill_discrete("WHO OSCI") +
  theme_bw() +
  theme(legend.position = "bottom")

tiff(filename = "../tmp/who_osci_over_time.tiff",
     height = 6,
     width = 6,
     units = 'in',
     res = 600,
...
### Table 7: WHO OSCI of Expired by Day
...
```{r}
tt <- dt1[!is.na(`DPPIV (pg/ml)`), ]

hist(tt$`DPPIV (pg/ml)`, 20)
summary(tt$`DPPIV (pg/ml)`)
wilcox.test(tt$`DPPIV (pg/ml)` ~ tt$`Study Group`)

hist(log10(tt$`DPPIV (pg/ml)`[tt$`Study Group` == "No COVID"]),
     20,
     lims = range(tt$`DPPIV (pg/ml)`))
hist(log10(tt$`DPPIV (pg/ml)`[tt$`Study Group` == "COVID"]),
     20,
     lims = range(tt$`DPPIV (pg/ml)`))
tt1 <- t.test(log10(tt$`DPPIV (pg/ml)`) ~ tt$`Study Group`)
tt1

10^tt1$estimate

tt2 <- t.test(tt$`DPPIV (pg/ml)` ~ tt$`Study Group`)
tt2

tt[, .(mu = mean(`DPPIV (pg/ml)`),
       se = sd(`DPPIV (pg/ml)`)/sqrt(.N)),
   by = `Study Group`]
...
tt[, .(mu = mean(`DPPIV (pg/ml)`),
   ...
   by = list(`Study Group`,
             tt$AnyDM)]
tt2 <- wilcox.test(`DPPIV (pg/ml)` ~ AnyDM,
                   data = tt[`Study Group` == "COVID", ])
tt2
```
...
```{r}
tt <- dt1[!is.na(`ACE2 (ng/ml)`), ]

hist(tt$`ACE2 (ng/ml)`, 20)
summary(tt$`ACE2 (ng/ml)`)
wilcox.test(tt$`ACE2 (ng/ml)` ~ tt$`Study Group`)

hist(log10(tt$`ACE2 (ng/ml)`[tt$`Study Group` == "No COVID"]),
     20,
     lims = range(tt$`ACE2 (ng/ml)`))
hist(log10(tt$`ACE2 (ng/ml)`[tt$`Study Group` == "COVID"]),
     20,
     lims = range(tt$`ACE2 (ng/ml)`))
t.test(log10(tt$`ACE2 (ng/ml)`) ~ tt$`Study Group`)

tt2 <- t.test(tt$`ACE2 (ng/ml)` ~ tt$`Study Group`)
tt2

tt[, .(mu = mean(`ACE2 (ng/ml)`),
       se = sd(`ACE2 (ng/ml)`)/sqrt(.N)),
   by = `Study Group`]
...
tt[, .(mu = mean(`ACE2 (ng/ml)`),
   ...
   by = list(`Study Group`,
             tt$AnyDM)]
tt2 <- wilcox.test(`ACE2 (ng/ml)` ~ AnyDM,
                   data = tt[`Study Group` == "COVID", ])
tt2
```
              ...
              function(a) {
                try({
                  ...
                  rbindlist(list(data.table(Model = "Logistic",
                                            Names = rownames(s1$Firstpart.model$coefficients)[-1],
                                 ...
                                 data.table(Model = "Gaussian",
                                            Names = rownames(s1$Firstpart.model$coefficients)[-1],
                                            Estimated = "Log2(Concentration [pg/mL] + 1) Difference",
...
out$Comparisons <- factor(out$Names,
                          levels = c("grpCOVID/DM",
                                     "grpCOVID/No DM",
                                     "grpNo COVID/DM"),
                          labels = c("COVID/DM - No COVID/No DM",
                                     "COVID/No DM - No COVID/No DM",
...
p1 <- ggplot(data = out[Model == "Gaussian", ],
  ...
  geom_point(shape = 21,
             size = 3) +
  scale_x_continuous("Log2(Concentration [pg/mL] + 1) Difference") +
  ggtitle("Gaussian Part of the Model") +
  theme_bw() +
  theme(legend.position = "none")

tiff(filename = "../tmp/elisa_two_part_gaussian_lo2ratio.tiff",
     height = 8,
     width = 9,
     units = 'in',
     res = 600,
...
p1 <- ggplot(data = out[Model == "Gaussian", ],
  ...
  geom_point(shape = 21,
             size = 3) +
  scale_x_continuous("Ratio of (Concentration [pg/mL] + 1)") +
  ggtitle("Gaussian Part of the Model") +
  theme_bw() +
  theme(legend.position = "none")

tiff(filename = "../tmp/elisa_two_part_gaussian_ratio.tiff",
     height = 8,
     width = 9,
     units = 'in',
     res = 600,
...
p2 <- ggplot(tmp[gene_name == "IFI27", ],
  ...
  scale_y_continuous("Number of hits",
                     ...
                     labels = (2^seq(from = 1,
                                     to = 20,
                                     by = 2)) - 1) +
  scale_fill_manual("In-Hospital Death",
                    breaks = c("No",
                               "Yes"),
                    values = c("green",
                               "red")) +
  ggtitle("IFI27 expression") +
  theme_bw() +
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle = 45,
                                   hjust = 1))
...
p3 <- ggplot(tmp[gene_name == "IFI27", ],
  ...
  scale_y_continuous("Number of hits",
                     ...
                     labels = (2^seq(from = 1,
                                     to = 20,
                                     by = 2)) - 1) +
  scale_fill_manual("Critical Care",
                      breaks = c("No",
                                 "Yes"),
                    values = c("green",
                               "blue")) +
